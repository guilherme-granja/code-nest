...
export function AttachMenu({ sessionId, connectionId }: { sessionId: string; connectionId: string }) {
  ...
  return (
    <div className="relative">
      <button className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-base text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200" title="Anexar" onClick={() => setOpen((o) => !o)}>+</button>
      {open && (
